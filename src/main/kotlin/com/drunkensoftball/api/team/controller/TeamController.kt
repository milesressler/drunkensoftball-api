...
class TeamController {
    ...
    fun getTeam(@AuthenticationPrincipal user: User, @PathVariable(value = UUID) teamUuid: String): Team {

        return teamService.getTeam(user, teamUuid)
    }

    @RequestMapping(value = [URL_TEAM_UUID], method = [DELETE])
    fun deleteTeam(@AuthenticationPrincipal user: User, @PathVariable(value = UUID) teamUuid: String) {
